...
###### 휴대폰 접속과 QR (2026-09-13)

시연에서 관람자가 자기 휴대폰으로 사용자 화면을 보게 하려면 서버가 `127.0.0.1`
바깥으로 열려야 한다. `--host 0.0.0.0`을 주면 열리고, **기본값은 그대로
`127.0.0.1`이다.** 옵션 없이 실행하면 이전과 동작이 완전히 같다.

`/qr`을 열면 서버가 자기 LAN 주소를 읽어 `http://<주소>:8765/user` 를 QR로 그린다.
주소는 UDP 소켓을 라우팅 테이블에 물어보는 방식으로 얻는다 — 패킷을 보내지 않으므로
인터넷이 없는 핫스팟에서도 동작한다. 네트워크를 갈아타면 새로고침만 하면 된다.
QR 생성은 `segno`(순수 파이썬, 자체 의존성 없음)를 쓴다. 없으면 그 사실을
`/qr`에서 알려주고 나머지 기능은 그대로 돈다.

**열면서 반드시 같이 잠가야 한다.** `0.0.0.0`으로 열면 현장 Wi-Fi의 누구나
`/admin`에 들어와 노드에 CFG를 보낼 수 있다. 원격 요청은 `PUBLIC_PATHS`
(`/`, `/user`, `/api/sites`)만 통과시키고 **POST는 전부 거부한다.** QR을 찍고
`/`로 들어온 휴대폰은 첫 화면 대신 사용자 화면을 바로 받는다 — 첫 화면에는
관리자 링크가 있다.

| 경로 | 맥 | 휴대폰 |
|---|---|---|
| `/`, `/user`, `/api/sites` | 열림 | 열림 |
| `/admin`, `/survey`, `/qr`, `/api/state` | 열림 | **403** |
| 모든 POST | 열림 | **403** |

현장에서 실패하는 원인은 대개 코드가 아니다. **DHCP로 받은 주소는 재접속하면
바뀌고**, 학교·전시장 Wi-Fi는 기기 간 통신을 막는 경우가 많다(client isolation).
그래서 시연에는 **휴대폰 핫스팟**을 쓴다 — 격리가 없고 주소도 예측 가능하다.
`0.0.0.0`으로 처음 켤 때 macOS 방화벽이 수신 허용을 묻는다.
